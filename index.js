...
function get20s(arr){
  ...
  for (let i = 0; i < arr.length; i++) {
    const year1 = parseInt(arr[i].years.split(" - ")[0])
    const year2 = parseInt(arr[i].years.split(" - ")[1])
...
function addArtist(arr){
    arr.push({
      id: 20,
      name: "Allan",
      years: Math.abs((new Date(Date.now() - new Date('1986-03-17').getTime())).getFullYear() - 1970),
      genre: "web dev",
      nationality: "Brazilian / American",
      bio: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus deleniti ad corporis? Laboriosam non ducimus explicabo molestias, quia provident inventore, porro nihil, asperiores deserunt consequuntur ea ab vitae commodi maxime."
    })
    return arr
  }
